...
class SupabaseRelationalRepository:
    ...
    def create_customer(self, company_id: int, customer_name: str, phone_number: str) -> dict:
        ...
        context = CustomerContext(phone_number=str(phone_number or "").strip(), company_id=company_id)
        return create_customer(context, customer_name, phone_number)
    ...
    def get_booking_by_id(self, company_id: int, booking_id: int, service_type: str = "GROOMING") -> dict:
    ...
    def update_loyalty_points(self, company_id: int, customer_id: int, new_balance: int) -> dict:
        from relational_actions import update_loyalty_points

        context = CustomerContext(company_id=company_id)
        context.resolved_customer_id = int(customer_id)
        return update_loyalty_points(context, int(new_balance))
    ...
    def create_loyalty_transaction(
        ...
        *,
        ...
        from relational_actions import create_loyalty_transaction

        context = CustomerContext(company_id=company_id)
        return create_loyalty_transaction(
            context,
