"""
Rule-based metadata tagging for RAG chunks: dataset_type, pet_type,
service_type, service_info.

Ported from the retired ai-backend/app/services/metadata_tagging.py +
metadata_schema.py (pre-restructure commit 2d5ecb4^) — this is the same
keyword/frame classification the production BGE-Large index was built with,
kept verbatim rather than re-derived so re-indexed documents tag the same
way as the existing 319 chunks already in chunks_bge_large.
"""
...
_DOG_TERMS = ("dog grooming", "dog service", "dog size", "dogs", "dog", "puppy", "puppies")
_CAT_TERMS = ("cat grooming", "cat service", "cats", "cat", "kitten", "kittens")
_GROOMING_TERMS = (
    "grooming", "bath", "haircut", "hair cut", "nail trim", "nail trimming",
    "fur trimming", "grooming package", "grooming price", "dog grooming",
    "cat grooming", "spa", "trimming",
)
_BOARDING_TERMS = (
    "boarding", "overnight stay", "overnight care", "overnight", "check-in",
    "check in", "checkin", "check-out", "check out", "checkout",
    "boarding price", "boarding rules", "boarding rule",
)
_DAYCARE_TERMS = (
    "daycare", "day care", "day-care", "playtime", "play time", "daily care",
    "daycare price", "daycare rules", "daycare rule",
)
_GENERAL_TERMS = (
    "cancellation", "cancel", "refund", "loyalty", "loyalty points", "rewards",
    "redemption", "payment policy", "business hours", "operating hours",
    "general terms", "terms and conditions", "all services", "all service",
)
_LOYALTY_FRAMES = (
    "earn points", "redeem points", "points balance", "points expiry",
    "points be used", "points for", "points from", "use points", "using points",
    "loyalty points", "loyalty program", "membership rewards", "membership reward",
    "membership package", "point package", "points package", "how many points",
    "points do i have", "eligible services", "non-refundable", "non refundable",
    "no cash refund", "no cash refunds", "topped-up", "topped up", "top up points",
)
_LOYALTY_ANCHORS = ("loyalty", "membership", "member", "redeem", "redemption", "rewards", "reward", "points")
_CANCEL_FRAMES = (
    "cancel my", "cancel the", "cancel after", "can i cancel", "cancellation",
    "cancel booking", "cancel my booking", "cancel my appointment", "cancel appointment",
    "to cancel", "need to cancel", "cancel last", "cancel because", "cancelled",
    "canceled", "after cancellation", "no-show", "no show", "refund if i cancel",
    "refund if cancel", "cash refund for cancelled", "cash refund for canceled",
    "late pickup", "late check-out", "late checkout", "pick up my pet on time",
    "cannot pick up", "fully booked", "advance booking",
)
_RESCHEDULE_FRAMES = (
    "reschedule", "change my booking", "change booking", "change my appointment",
    "change appointment", "change grooming date", "change the date",
    "move my booking", "move my appointment",
)
_SERVICE_PRIMARY_FRAMES = (
    "how much", "price", "prices", "pricing", "cost", "fee", "fees", "package",
    "packages", "what is your", "do you provide", "do you have", "services do you",
)
_BOOKING_OBJECT_CUES = (
    "appointment", "booking", "cancel my", "cancel the", "can i cancel",
    "cancel booking", "cancel appointment", "reschedule", "change my booking",
    "change booking", "change my appointment", "no-show", "no show",
    "refund if i cancel", "refund if cancel", "late pickup", "late check-out", "late checkout",
)
_LOYALTY_REFUND_CUES = (
    "no cash refund", "no cash refunds", "non-refundable", "non refundable",
    "points be refunded", "points refunded", "points refund", "point package", "points package",
)
_POINTS_REQUEST_CUES = (
    "how many points", "earn points", "redeem points", "points for", "points from",
    "points needed", "points do i have", "use points", "using points",
    "points balance", "points expiry", "points be used", "loyalty points",
)
...
def _has_loyalty_business_object(text: str) -> bool:
    ...
    if not has_anchor:
        return False
    loyalty_verbs = (
        "earn", "redeem", "use", "used", "using", "count for", "counts for",
        "valid", "expiry", "expire", "balance", "how many", "do i have",
        "program", "package", "eligible", "membership", "loyalty", "points",
        "reward", "rewards",
    )
...
def _infer_business_object(text: str) -> str:
    ...
    booking_as_object = (
        _has_any(text, _BOOKING_OBJECT_CUES)
        or _has_cancellation_signal(text)
        or _has_reschedule_signal(text)
    )
...
def _intent_from_object_and_action(*, text: str, business_object: str, service_context: str) -> tuple[str, float, str]:
    loyalty_hit = _has_loyalty_business_object(text) or business_object in ("points", "membership", "point_package")
    ...
    if len(active) <= 1:
        if business_object in ("points", "membership", "point_package") or active == ["loyalty"]:
            return ("loyalty", 0.92, f"Single loyalty topic; business object is {business_object}")
        if active == ["reschedule"] or (business_object == "booking" and reschedule_hit and not cancel_hit):
            return ("reschedule", 0.92, "Single reschedule topic; action applies to a booking")
        if active == ["cancellation"] or (business_object == "booking" and cancel_hit):
            return ("cancellation", 0.92, "Single cancellation topic; action applies to a booking")
        if active == ["service"] or business_object == "service":
            return ("service", 0.9, f"Single service topic; requesting info about {service_context}")
    ...
    if business_object == "booking":
        if reschedule_hit and not cancel_hit:
            return (
                ...
                + (f"; {service_context} is booking context" if service_context in ("grooming", "boarding", "daycare") else ""),
            )
        if cancel_hit:
            return (
                "cancellation", 0.88,
                "Main object is booking; action is cancellation"
                + (f"; {service_context} is booking context" if service_context in ("grooming", "boarding", "daycare") else ""),
            )
        if reschedule_hit:
            return ("reschedule", 0.85, "Main object is booking; action is reschedule")

    if business_object == "service" or (service_primary and not loyalty_hit and not cancel_hit and not reschedule_hit):
        return ("service", 0.86, f"Main object is the {service_context} service itself")
...
def classify_business_object(
    ...
    *,
    ...
    """Classify main intent from full semantic meaning (no fixed keyword priority)."""
    blob = _norm(
        " ".join(part for part in (text, main_header, sub_header, service_info, section_context) if str(part or "").strip())
    )
    ...
    main_intent, confidence, reason = _intent_from_object_and_action(
        text=blob, business_object=business_object, service_context=service_context
    )
    return {
...
def _short_topic_label(*, blob: str, pet_type: str, service_type: str, text: str = "") -> str:
...
def generate_service_info(
    *, sub_header: str = "", main_header: str = "", section_title: str = "", blob: str = "",
    pet_type: str = "all", service_type: str = "general", text: str = "",
) -> str:
    ...
    return _short_topic_label(blob=blob, pet_type=pet_type, service_type=service_type, text=text)
...
def tag_chunk_metadata(chunk: dict[str, Any]) -> dict[str, Any]:
    ...
    if classified["main_intent"] == "service" and classified["service_context"] in ("grooming", "boarding", "daycare"):
    ...
    if _has_any(
        header_blob,
        (
            "point package", "membership package", "loyalty", "eligible services",
            "point deduction", "point conversion", "point package validity",
            "membership validity", "misuse policy",
        ),
    ) or (
        "points" in header_blob and _has_any(header_blob, ("package", "redeem", "redemption", "membership"))
    ):
        service_type = "general"
        pet_type = "all"

    service_info = generate_service_info(
        sub_header=sub_header, main_header=main_header, section_title=section_title,
        blob=blob, pet_type=pet_type, service_type=service_type, text=text,
    )
