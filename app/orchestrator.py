...
class PawfectOrchestrator:
        ...
        try:
            if state is not None:
                ...
                if tool_call["name"] == "check_availability":
                    args = self._bind_selected_slot_to_availability_args(state, args, user_message)
                if tool_call["name"] == "create_booking":
                    ...
                    # If the customer just selected one of the verified active
                    # slots, keep create_booking on that exact date/time rather
                    # than trusting a model-reconstructed value. The write path
                    # still performs its own fresh DB availability check.
                    if state.selected_slot:
                        selected = state.selected_slot
                        incoming_service = str(args.get("service_type") or "").strip().upper()
                        selected_service = str(selected.get("service_type") or "").strip().upper()
                        compatible = not (incoming_service and selected_service and incoming_service != selected_service)
                        if compatible:
                            if selected.get("date") and not args.get("date"):
                                args = {**args, "date": selected["date"]}
                            if selected.get("slot") and not args.get("time"):
                                args = {**args, "time": selected["slot"]}
                            if selected.get("duration_minutes") not in (None, "") and not args.get("duration_minutes"):
                                args = {**args, "duration_minutes": selected["duration_minutes"]}
                            for key in ("check_out_date", "check_out_time"):
                                if selected.get(key) and not args.get(key):
                                    args = {**args, key: selected[key]}
    ...
    @staticmethod
    def _cache_offered_options(state, tool_name: str, result: dict, args: dict | None = None) -> None:
        """Cache customer-facing choices without mixing service options and slots.

        ``offered_options`` remains the generic backwards-compatible list used
        by cancel/reschedule ordinal handling. Booking flows additionally keep
        ``active_service_options`` and ``active_slots`` with enough context to
        make a later short reply such as "2:30pm" refer to the exact verified
        availability conditions that produced that slot.
        """
        if tool_name not in (
            "get_booking_service_options",
            "check_availability",
        ...
        if tool_name in ("cancel_booking", "reschedule_booking"):
            if result.get("status") == "ambiguous":
                ...
                if candidates:
                    ...
                    state.active_offer_kind = "booking_candidates"
            return
        ...
        if result.get("status") != "success" and not range_success:
            if result.get("status") in {"not_found", "error", "missing_information"}:
                if tool_name == "get_booking_service_options":
                    state.active_service_options = []
                else:
                    state.active_slots = []
                    state.selected_slot = None
                    state.availability_context = None
                state.offered_options = []
            return
        ...
        if tool_name == "get_booking_service_options":
            ...
            state.active_offer_kind = "service_options"
            state.offered_options = list(state.active_service_options)
            # A newly presented service catalogue invalidates any old slot list.
            state.active_slots = []
            state.selected_slot = None
            state.availability_context = None
            return
        ...
        if tool_name == "check_availability":
            service_type = str(data.get("service_type") or args.get("service_type") or "").upper()
            booking_date = str(data.get("booking_date") or data.get("requested_date") or args.get("date") or "")
            context = {
                ...
                "customer_id": args.get("customer_id") or state.customer_id,
                "preferred_staff": args.get("preferred_staff") or state.preferred_staff or "",
            }
            # For an exact request, all_available_slots is the authoritative
            # current alternatives list; available_slots is only the exact
            # requested slot (or []). For broad/period checks, available_slots
            # is already the correct presented subset.
            exact_scope = data.get("availability_scope") == "exact"
            source_slots = (data.get("all_available_slots") if exact_scope else data.get("available_slots")) or []
            state.active_slots = [
                {
                    "label": slot,
                    "slot": slot,
            ...
            state.availability_context = context
            state.active_offer_kind = "slots"
            state.offered_options = list(state.active_slots)

            requested_time = str(data.get("requested_time") or "").strip()
            if exact_scope and requested_time:
                target = normalize_time_to_slot(requested_time)
                selected = next(
                    (item for item in state.active_slots if normalize_time_to_slot(str(item.get("slot") or "")) == target),
                    None,
                )
                state.selected_slot = selected if data.get("requested_time_available") else None
            else:
                state.selected_slot = None
            return

        if tool_name == "check_availability_range":
            ...
            state.active_slots = [
                {
                    ...
                    "slot": slot,
                    "service_type": str(args.get("service_type") or "").upper(),
    ...
    def _sync_scenario_from_tool_call(cls, state, tool_name: str, result: dict) -> None:
        ...
        if confirmed and isinstance(result, dict) and result.get("status") == "success":
            ...
            state.active_service_options = []
            state.active_slots = []
            state.active_offer_kind = None
            state.selected_service_option = None
            state.selected_slot = None
            state.availability_context = None
    ...
    def _resolve_active_slot_reference(self, state, user_message: str) -> dict | None:
        """Resolve a short customer clock reply against the last verified slots.

        This is deliberately separate from ordinal option resolution. A reply
        like ``2:30pm`` after the assistant just offered 12:00..16:30 must bind
        to the exact verified slot context rather than making the model rebuild
        service/date/duration arguments from memory.
        """
        if not state.active_slots:
            return None
        clock = extract_time_from_message(user_message or "")
        target = normalize_time_to_slot(clock) if clock else ""
        if not target:
            return None
        matches = [
            item for item in state.active_slots
            if normalize_time_to_slot(str(item.get("slot") or "")) == target
        ]
        return matches[0] if len(matches) == 1 else None

    @staticmethod
    def _bind_selected_slot_to_availability_args(state, args: dict, user_message: str) -> dict:
        """Keep an exact re-check on the same conditions that produced the list.

        The customer is allowed to change only the clock time with a short
        reply. Pet/service/date/duration/room/checkout facts remain the same
        unless they explicitly change them. This prevents broad-list and exact
        re-check drift (the main source of fake-slot contradictions).
        """
        selected = state.selected_slot
        if not selected:
            return args
        clock = extract_time_from_message(user_message or "")
        if not clock or normalize_time_to_slot(clock) != normalize_time_to_slot(str(selected.get("slot") or "")):
            return args

        # Reuse the old slot context only when the model/customer has not
        # explicitly changed a higher-level booking condition. A message such
        # as "daycare next Friday at 2:30" must not be forced back onto an old
        # grooming 2:30 slot simply because the clock text happens to match.
        for key in ("service_type", "date", "room_type", "check_out_date", "check_out_time"):
            incoming = str(args.get(key) or "").strip().lower()
            cached = str(selected.get(key) or "").strip().lower()
            if incoming and cached and incoming != cached:
                state.selected_slot = None
                return args

        merged = dict(args)
        for key in ("service_type", "date", "duration_minutes", "room_type", "check_out_date", "check_out_time", "customer_id"):
            value = selected.get(key)
            if value not in (None, "") and merged.get(key) in (None, ""):
                merged[key] = value
        merged["time"] = selected.get("slot") or clock
        return merged
    ...
    def _apply_tool_result_state(self, state, tool_call: dict, result) -> None:
        ...
        self._cache_offered_options(state, tool_name, result, args)
    ...
    def invoke_with_trace(self, company_context: dict, state, user_message: str):
        ...
        active_slot_selection = self._resolve_active_slot_reference(state, user_message)
        resolved_selection = active_slot_selection or self._resolve_ordinal_reference(state, user_message)
        if active_slot_selection is not None:
            state.selected_slot = dict(active_slot_selection)
        elif resolved_selection is not None and state.active_offer_kind == "service_options":
            state.selected_service_option = dict(resolved_selection)
            # Selecting a different package invalidates any previously shown
            # slots; those were verified under another service-option context.
            state.active_slots = []
            state.selected_slot = None
            state.availability_context = None
