...
class ConversationState:
    ...
    # Dedicated booking-choice memory. ``offered_options`` is still kept for
    # cancel/reschedule candidate lists and backwards compatibility, but it
    # must no longer be the only place where service choices and verified
    # availability live: those two lists have different lifecycles.
    active_service_options: list[dict[str, Any]] = field(default_factory=list)
    active_slots: list[dict[str, Any]] = field(default_factory=list)
    active_offer_kind: str | None = None  # service_options | slots | booking_candidates
    selected_service_option: dict[str, Any] | None = None
