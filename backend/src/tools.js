  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        booking_date: { type: "string", description: "grooming/daycare, format YYYY/M/D" },
        ...
        check_in_date: { type: "string", description: "boarding only, format YYYY/M/D" },
        check_out_date: { type: "string", description: "boarding only, format YYYY/M/D" },
...
function todayStamp() {
  const d = new Date();
  return { date: `${d.getFullYear()}/${d.getMonth() + 1}/${d.getDate()}`, time: d.toTimeString().slice(0, 8) };
}
